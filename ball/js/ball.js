class Ball {
    ...
    calcCollide() {
        ...
        let collide = checkNextInterpoint(start, this.dir, this.ignores, this.hit);
        this.collide = collide;
        // 虚线物体或者当前为穿透球，需要记录正在那个敌方体内，再次碰撞其他物体前不会反复计算碰撞伤害
        if (this.collide.line && (!this.collide.line.solid || ldata.isThrough)) {
            this.hit = this.collide.line.mid;
        } else {
            this.hit = 0;
        }
